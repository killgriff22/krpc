...
class Screen(canvas):
            ...
            for i, flag in enumerate(self.events):
                if flag.is_set():
                    self.program.stdin.write(
                        list(keys.items())[i][1].encode("utf-8")+b"\n")
                    flag.clear()
    ...
    def program_thread(self, program: str, args: list[str] = []):
        self.Program_Thread = Thread(
            target=self.capture_program, args=(program, args))
        ...
        while True:
            ...
            if type(event) == asciimaticsEvent.KeyboardEvent:
                ...
                if event.key_code in keys:
                    self.screens[self.screens.index(screen)].events[list(
                        keys.keys()).index(event.key_code)].set()
